            ...
            if(appData.isReadComplete == true)
            {
                ...
                for(ii=0; ii<appData.readLength; ii++) {
                    if (appData.readBuffer[ii] == '\n' || appData.readBuffer[ii] == '\r'){
                        ...
                        txFlag = 1;
                        pos = 0;
                        sscanf(rx,"%d %d %d %d %d %d",&sp,&c1,&c2,&c3,&stop,&err);
                    }
                    else {
...
void CONTROLLER(int c1, int c2, int stop,int err)
{
    if(stop||err){LSPD = 0;RSPD = 0;LFOR = 1; RFOR = 1;}
    else
    {
        //int err1 = ((c1*2+c2)/3)-MIDDLE;
        int err1 = c1-MIDDLE;
        if (err1>0)
        {
            LFOR = 1; RFOR = 1;
            LSPD = MAX;
            RSPD = MAX*(MIDDLE-err1)/MIDDLE;
        }
        else
        {
            LFOR = 1; RFOR = 1;
            LSPD = MAX*(MIDDLE+err1)/MIDDLE;
            RSPD = MAX; 
        }
    }
}
